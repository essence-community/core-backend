...
/*##filter.cl_available*/ and m.cl_available = (cast(:json as jsonb)->''filter''->>''cl_available'')::bigint/*filter.cl_available##*/
/*##filter.cv_version_api*/ and REGEXP_REPLACE(m.cv_version_api, ''^(\d+\.\d+).*'',''\1'') = REGEXP_REPLACE((cast(:json as jsonb)->''filter''->>''cv_version_api''), ''^(\d+\.\d+).*'',''\1'') /*filter.cv_version_api##*/
order by &SORT
