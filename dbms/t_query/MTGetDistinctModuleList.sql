...
from
    (
        ...
        from
            (
                select
                    distinct cv_name, ck_view
                from
                    s_mt.t_module
            ) as m
        left join s_mt.t_module am on
            m.cv_name = am.cv_name
            and m.ck_view = am.ck_view
            and am.cl_available = 1
    ) as dm
where &FILTER
/*##filter.ck_id*/ and dm.ck_id = :json::json#>>''{filter,ck_id}''/*filter.ck_id##*/
/*##filter.ck_view*/ and dm.ck_view = :json::json#>>''{filter,ck_view}''/*filter.ck_view##*/
/*##filter.g_ck_view*/ and dm.ck_view = :json::json#>>''{filter,g_ck_view}''/*filter.g_ck_view##*/
order by &SORT
