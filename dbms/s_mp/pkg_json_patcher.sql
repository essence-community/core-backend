...
ALTER SCHEMA pkg_json_patcher OWNER TO s_mp;
...
CREATE FUNCTION pkg_json_patcher.f_modify_patch(pv_user character varying, pk_session character varying, pc_json jsonb) RETURNS character varying
    ...
    SET search_path TO 's_mt', 'pkg_json_patcher', 'pkg_patcher', 'public'
    AS $$
declare
  -- переменные пакета
  gv_error sessvarstr;
  ...
  -- переменные функции
  pot_create_patch  s_mt.t_create_patch;
  vv_action varchar(1);
begin
...
ALTER FUNCTION pkg_json_patcher.f_modify_patch(character varying, character varying, jsonb) OWNER TO s_mp;