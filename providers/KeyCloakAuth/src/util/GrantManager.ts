...
export class GrantManager {
    ...
    getAccount() {
        ...
        return this.userInfo.apply(this, arguments as any);
    }
    ...
    validateToken(token, expectedType) {
        return new Promise(async (resolve, reject) => {
            if (!token) {
                ...
                token.content.iss !== this.proxyUrl
            ) {
    ...
    loginUrl(uuid, redirectUrl) {
        let url =
            ...
            "&scope=" +
            encodeURIComponent(this.scope ? "openid " + this.scope : "openid") +
            "&response_type=code";
...
const createHandler =
    (manager: GrantManager, isHeader = false) =>
    async (resolve, reject, json, headers) => {
        try {
            resolve(
                isHeader
                    ? [await manager.createGrant(json), headers]
                    : await manager.createGrant(json),
            );
        } catch (err) {
            reject(err);
        }
...
const postOptions = (manager: GrantManager, path?: string) => {
    ...
    const opts = URL.parse(
        realPath.startsWith("http")
            ? realPath
            : (manager.proxyUrl || manager.realmUrl) + realPath,
    ) as any;
    opts.headers = {
        ...
        axios
            ...
            .then((res) => {
                manager.logger.debug(
                    "Status: %s, header: %j, response %j",
                    res.status,
                    res.headers,
                    res.data,
                );
                if (res.status < 200 || res.status > 299) {
                    manager.logger.warning(res.data);
                    return reject(
                        new Error(
