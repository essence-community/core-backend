...
export default class RestTransformProxy extends NullProvider {
    ...
    public async callRequest(
        ...
        optionsRequest.forEach((key: string) => {
            if (Object.prototype.hasOwnProperty.call(config, key)) {
                (params as any)[key] = (config as any)[key];
            }
        });
        ...
        if (typeof params.proxy === "string") {
            const proxy = (params.proxy as string).startsWith("{")
                ? JSON.parse(this.params.proxy as string)
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (config.proxyResult) {
                    ...
                    validHeader.find((key) => (ctHeader as string).startsWith(key as string))
                ) {
                    response.data.on("error", (err: any) => {
                        if (err) {
                            gateContext.error(
                    ...
                    if (
                        ...
                        config.resultPath === "" ||
                        !(ctHeader as string).startsWith("application/json")
                    ) {
                    ...
                    } else if ((ctHeader as string).startsWith("application/json")) {
                        ...
                        result.map((item, index) => {
                            ...
                            return parserRowResult.runer({
                                get: (key: string, isKeyEmpty: boolean) => {
                                    ...
                                    return (
                                        rowParam[key as keyof typeof rowParam] || (isKeyEmpty ? "" : key)
                                    );
                                },
                            });
                        }),
