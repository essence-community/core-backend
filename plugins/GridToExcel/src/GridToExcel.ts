    ...
    public async afterQueryExecutePerform(
        ...
        const configRender = {
            "template":  {  
                ...
                "content": "{{#xlsxAdd \"xl/worksheets/sheet1.xml\" \"worksheet.cols\"}}" +
                           "     <cols>" +
                           jsonbc.columns.reduce((all, col, colIndex) => `${all}\n<col customWidth="0" bestFit="1" width="20" max="${colIndex+1}" min="${colIndex+1}"/>`, "") +
                           "     </cols>" +
                           "{{/xlsxAdd}}" +
                           "{{#xlsxAdd \"xl/worksheets/sheet1.xml\" \"worksheet.sheetData[0].row\"}}" +
                           "     <row r=\"1\" customHeight=\"0\" bestFit=\"1\">" +
                           jsonbc.columns.reduce((all, col, colIndex) => `${all}\n<c r="${getColumnName(colIndex+1)}1" t="inlineStr"><is><t>${col.cv_displayed}</t></is></c>`, "") +
                           "     </row>" +
                           "{{/xlsxAdd}}" +
                           "{{#each rows}}" +
                           "  {{#xlsxAdd \"xl/worksheets/sheet1.xml\" \"worksheet.sheetData[0].row\"}}" +
                           `     <row r="{{inc @index}}" customHeight="0" bestFit="1">` +
                           jsonbc.columns.reduce((all, col, colIndex) => `${all}\n<c r="${getColumnName(colIndex+1)}{{inc @index}}" t="inlineStr"><is><t>{{${col.column}}}</t></is></c>`, "") +
                           "     </row>" +
                           "  {{/xlsxAdd}}" +
                           "{{/each}}" +
                           (jsonbc.cv_displayed ? 
                ...
                "xlsx":  {  
                  "templateAsset":  {  
                    "content": fs.readFileSync(path.join(__dirname, "assets", "book.xlsx")).toString("base64"),
                    "encoding":"base64"
                   }
                }  
              },  
