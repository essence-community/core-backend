...
COMMENT ON COLUMN ${user.table}.t_role_action.ck_id IS 'Идентификатор';
COMMENT ON COLUMN ${user.table}.t_account_role.ck_id IS 'Идентификатор';
...
ALTER TABLE ${user.table}.t_account_info ADD CONSTRAINT cin_u_account_info_1 UNIQUE (ck_d_info,ck_account);

--changeset artemov_i:CORE-870_add_INDEX dbms:postgresql
CREATE UNIQUE INDEX cin_u_account_2 ON ${user.table}.t_account (UPPER(cv_login));

--changeset artemov_i:CORE-225 dbms:postgresql
CREATE TRIGGER notify_account_event
AFTER INSERT OR UPDATE OR DELETE ON ${user.table}.t_account
  FOR EACH ROW EXECUTE PROCEDURE notify_event();
CREATE TRIGGER notify_account_role_event
AFTER INSERT OR UPDATE OR DELETE ON ${user.table}.t_account_role
  FOR EACH ROW EXECUTE PROCEDURE notify_event();
CREATE TRIGGER notify_account_info_event
AFTER INSERT OR UPDATE OR DELETE ON ${user.table}.t_account_info
  FOR EACH ROW EXECUTE PROCEDURE notify_event();
CREATE TRIGGER notify_role_action_event
AFTER INSERT OR UPDATE OR DELETE ON ${user.table}.t_role_action
  FOR EACH ROW EXECUTE PROCEDURE notify_event();

--changeset artemov_i:CORE-1035 dbms:postgresql
INSERT INTO ${user.table}.t_action (ck_id,cv_name,cv_description,ck_user,ct_change)
	VALUES (99999,'Анонимный доступ','Анонимный доступ','4fd05ca9-3a9e-4d66-82df-886dfa082113','2020-03-04 19:15:11.069');
INSERT INTO ${user.table}.t_role (ck_id,cv_name,cv_description,ck_user,ct_change)
...
INSERT INTO ${user.table}.t_account (cv_surname,cv_name,cv_login,cv_hash_password,ck_id,ck_user,ct_change,cv_timezone,cv_salt)
...
INSERT INTO ${user.table}.t_role_action (ck_id,ck_action,ck_user,ct_change,ck_role)
...
INSERT INTO ${user.table}.t_account_role (ck_id,ck_role,ck_user,ct_change,ck_account)
...
CREATE TABLE ${user.table}.t_auth_token (
...
COMMENT ON COLUMN ${user.table}.t_auth_token.ck_id IS 'Токен';
COMMENT ON COLUMN ${user.table}.t_auth_token.ct_start IS 'Дата начала работы';
COMMENT ON COLUMN ${user.table}.t_auth_token.ct_expire IS 'Дата истечения';
COMMENT ON COLUMN ${user.table}.t_auth_token.ck_account IS 'ИД пользователя, для которого создан токен';
COMMENT ON COLUMN ${user.table}.t_auth_token.cl_single IS 'Признак того, что токеном можно воспользоваться только 1 раз';
COMMENT ON COLUMN ${user.table}.t_auth_token.ck_user IS 'ИД пользователя аудит';
COMMENT ON COLUMN ${user.table}.t_auth_token.ct_change IS 'Время модификации';
INSERT INTO ${user.table}.t_action (ck_id,cv_name,cv_description,ck_user,ct_change)
...
DELETE FROM ${user.table}.t_account_role tar WHERE tar.ck_account = '4fd05ca9-3a9e-4d66-82df-886dfa082113' and tar.ck_role = 'ea196953-643d-4666-9a0d-b37689837e2f';
