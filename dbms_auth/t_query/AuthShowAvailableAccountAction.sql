...
--changeset artemov_i:AuthShowAvailableAccountAction.sql dbms:postgresql runOnChange:true splitStatements:false stripComments:false
INSERT INTO s_mt.t_query (ck_id,cc_query,ck_provider,ck_user,ct_change,cr_type,cr_access,cv_description)
VALUES ('AuthShowAvailableAccountAction','/*AuthShowAvailableAccountAction*/
select /*Pagination*/
...
left join t_account_action ra
 ...
 where ra.ck_account is null
 ) t
  where &FILTER
 order by &SORT
offset &OFFSET rows
 fetch first &FETCH rows only','authcore','-11','2019-08-13 18:30:00.000','select','po_session','Список доступных действий')
