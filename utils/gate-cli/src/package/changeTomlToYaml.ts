...
export async function changeTomlToYaml() {
    ...
    const isDelete = isTrue(
        await questionReadline("Delete old property(no):", "no"),
    );
    if (isEmpty(dir)) {
        process.exit(1);
    }
    fs.readdirSync(dir).forEach((key) => {
        if (key.endsWith("toml")) {
            ...
            const temp = TOML.parse(
                fs.readFileSync(path.join(dir, key)).toString(),
            );
            fs.writeFileSync(
                path.join(dir, key.replace(".toml", ".yaml")),
                YAML.dump(temp.data),
            );
            if (isDelete) {
